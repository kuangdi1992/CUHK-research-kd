...
parser.add_argument('--file-name',default='AlexNet_Var_momentum',type=str)
parser.add_argument('--init-method',default='tcp://172.18.233.41:22323',type=str)
...
def gradient_execute(net):
    ...
    for param in net.parameters():
        ...
        num = temp1.size
        pnum = []
        num1 = temp1[temp1 > 0].size
        pnum.append(num1 / num)
        pnum.append((num - num1) / num)
        hx = 0
        for i in range(2):
            if pnum[i] == 0:
                hi = 0
            else:
                hi = -pnum[i] * np.log2(pnum[i])
            hx = hx + hi
        if hx == 0:
            topn = torch.topk(abs(temp.view(1, -1)),
                              int(temp.nelement() * 0.01) if int(temp.nelement() * 0.01) != 0 else 1)
...
def run():
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=2, verbose=True, min_lr=1e-5)
    net.train()
    net = net.cuda()
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_loader,0):
            ...
            for param1,param2,param3 in zip(u_kt,net.parameters(),v_kt) :
                param1.grad.data = param1.grad.data*momentum + copy.deepcopy(param2.grad.data)
                param3.grad.data = param3.grad.data + copy.deepcopy(param1.grad.data)
                param2.grad.data = copy.deepcopy(param3.grad.data)

            paralist = gradient_execute(net)
            optimizer.step()
            for para1, para3 in zip(paralist, v_kt):
                para3.grad.data = para1
